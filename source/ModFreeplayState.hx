...
class ModFreeplayState extends MusicBeatState
{
	var songs:Array<SongMetadataMod> = [];
	...
	var CatagoryTxt:FlxText;
	var diffText:FlxText;
	var lerpScore:Float = 0;
	var intendedScore:Int = 0;

	private var grpSongs:FlxTypedGroup<Alphabet>;
	public static var coolColors = [];
	...
	{
		CatagoryTxt = new FlxText("Press A and D to move arround catagorys");
		CatagoryTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		CatagoryTxt.screenCenter(X);
		CatagoryTxt.y = 1010;
		add(CatagoryTxt);
		...
		var freeplayData:FreeplayJsonDataMod = Json.parse(Assets.getText(Paths.json("freeplay")));
		...
		for(data in freeplaySongs)
		{
			...
			songs.push(new SongMetadataMod(song, data.week, data.icon, data.difficulties, FlxColor.fromString(data.color)));
		}
	...
	{
		songs.push(new SongMetadataMod(songName, weekNum, songCharacter, difficulties));
	}
...
class SongMetadataMod
{
	...
	public function new(song:String, week:Int, songCharacter:String, ?difficulties:Array<String> = null, ?songColor:Null<Int> = null)
	{
		...
		if(songColor != null)
			...
			this.songColor = ModFreeplayState.coolColors[week];
	}
}

typedef FreeplayJsonDataMod = {
	var songs:Array<SongJsonDataMod>;
}

typedef SongJsonDataMod = {
	var name:String;
	var icon:String;
